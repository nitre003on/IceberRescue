...
class LINIE_ERKENNEN(Thread):
    ...
    def linie_aussuchen_unten(self, dieser_frame, offset, LE_unten):             
    ...
    def run(self):
        while  not self.stop:
            if len(self.cam.frame) > 0:
                dieser_frame = self.cam.frame
...
class LINIE_FOLGEN(Thread):
    ...
    def fahren_US_OS_LS_RS(self):
        fahren_US_OS()

    def fahren_US_LS_RS(self):
        print("fahren_US_LS_RS")
        if POINT_2[1] > POINT_3[1]:
    ...
    def run(self):
        while not self.stop:
            if US and OS and not LS and not RS:
                ...
                fahren_US_OS()
            center_line(self.le.dieser_frame)
            cv2.imshow("Maze Runner", self.le.dieser_frame)
